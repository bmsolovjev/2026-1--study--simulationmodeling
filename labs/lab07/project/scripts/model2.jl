...
# working_count  число исправных машин (работающие + резерв)
...
@resumable function machine(
    ...
    while true
        ...
        # Отказ машины: исправных стало меньше на 1
        ...
        if state(get_spare) != ConcurrentSim.idle
            ...
            # Запасная запущена — число исправных не меняется (одна сломалась, одну запустили)
            push!(working_machines, (now(env), working_count))
        else
            push!(working_machines, (now(env), working_count))
        ...
        # Машина вернулась из ремонта в резерв — исправных стало больше
        global working_count
        working_count += 1
        push!(working_machines, (now(env), working_count))
    end
...
# Средние показатели за весь прогон
